...
impl Clone for AigEdge {
    fn clone(&self) -> AigEdge{
        AigEdge {flip: self.flip, node: self.node.clone()}
    }
}
...
pub fn truen() -> AigEdge {
    AigEdge { flip:false, node: Rc::new(AigNode::TrueNode)}
}

pub fn var(s: String)-> AigEdge {
    AigEdge {flip : false, node : Rc::new(AigNode::VarNode(s)) }
}

pub fn and(a : &AigEdge, b : &AigEdge) -> AigEdge {
    if a.node.as_ref() == &AigNode::TrueNode || b.node.as_ref() == &AigNode::TrueNode {
        if a == &truen(){
            b.clone()
        } else if a == &not(&truen()) {
            not(b)
        ...
        AigEdge { flip: false, node : Rc::new(AigNode::AndNode(a.clone(), b.clone()))}
    }
}

pub fn not(a : &AigEdge)-> AigEdge{
    AigEdge { flip: !a.flip, node:a.node.clone()}
}
